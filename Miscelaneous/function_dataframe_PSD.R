fnc.dataframe_PSD <- function(ZZZ_Hourly_PSD,ZZZ_Weekly_PSD,ZZZ_LoadSteps,ZZZ_VLookUp_Holidays){
  ...
  #   mutate()
    ...
    # )
    # 
    mutate(diff_perc = (PSDh/PSD)-1,diff_nom = (PSDh-PSD)) %>% 
    select(SubMkt,Date,`Year/Month`,Hour_LoadStep,WeekDay_LoadStep,WDay,LoadStep,
           PSD,PSDh,diff_perc,diff_nom,everything())
